...
import { PAYSWITCH_MERCHANTID, PAYSWITCH_TEST_BASEURL } from 'src/constants';
...
export class PsmobilemoneyService {
  ...
  public transferMobilemoney(transDto: TransferMobileMoneyDto): Observable<AxiosResponse<TransferMobileMoneyDto>> {
    const {
      description,
      recipientMsisdn,
      amount,
  ...
  public mobilemoneyPayment(transDto: PayMobileMoneyDto): Observable<AxiosResponse<PayMobileMoneyDto>> {
    const {
      clientReference,
      customerMsisdn,
      amount,
    ...
    return this.httpService.post(configs.url,configs.body, { httpsAgent: configs.agent, headers: configs.headers }).pipe(
      map(mpRes => {
        this.logger.verbose(`RECEIVE MONEY server response => ${mpRes.data}`);
        // if (res.data.Status === 0) {
        //     this.logger.error(`debit wallet service response STATUS =  ${JSON.stringify(res.data.Status)}`);
        //     this.logger.error(`service response MESSAGE =  ${JSON.stringify(res.data.Message)}`);
        //     this.logger.error(`service response DETAILS = ${JSON.stringify(res.data.Details)}`);
      ...
      catchError(mpError => {
        this.logger.error(`ERROR DEBIT WALLET => ${mpError.data}`);
        return mpError.data;
      }),
    );
  }
